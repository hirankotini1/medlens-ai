...
[![Tests](https://img.shields.io/badge/Tests-45%2F45%20Passing-success.svg)](#running-automated-tests)
...
[![AI Integration](https://img.shields.io/badge/AI--Integration-OpenRouter%20%7C%20Multi--Format-teal.svg)](docs/ai_report_analyzer.md)

**Nexus Pathology** is a web-based digital pathology laboratory management platform combined with an experimental machine-learning decision-support backend serving five validated diagnostic models and an intelligent **AI Health Report Analyzer** powered by OpenRouter AI.

---

## Key Features

1. **AI Health Report Analyzer:** Ingests complete diagnostic reports (PDF, CSV, JPG, PNG, TXT), automatically parses numerical biomarkers, allows interactive inline parameter editing, and provides comprehensive clinical intelligence synthesizing OpenRouter AI and validated Scikit-Learn models.
2. **Patient Portal:** Authenticated, privacy-preserving portal where patients can access their official laboratory reports, view biological reference intervals and clinical flags (Normal, Low, High, Critical), read doctor remarks, and view separate ML analysis summaries.
3. **Administrative & Laboratory Staff Portal:** Operations management for registering patients, authoring structured clinical reports with dynamic parameter panels, managing report lifecycles (Draft vs. Finalized), and triggering on-demand ML decision support.
4. **Strict Report Decoupling:** Official medical findings (`lab_reports`) remain immutable and uncorrupted by ML inferences. All generated predictions are recorded separately in an independent audit log (`ml_predictions` and `report_analyses`).
5. **Five Validated ML Pipelines:** Pre-trained and serialized Scikit-Learn models with zero data leakage, evaluated via 5-fold cross-validation.
6. **Direct ML Sandbox:** Exploratory testing playground with realistic clinical presets and a secure drag-and-drop microscopy image upload interface for Malaria detection.
7. **Cybersecurity & Privacy Controls:** Cryptographic PBKDF2-HMAC-SHA256 password and PIN hashing, HMAC-signed session tokens, Role-Based Access Control (RBAC), and strict defenses against Insecure Direct Object References (IDOR).
8. **Print & PDF Layout:** Native `@media print` styling that formats findings into a physical laboratory report sheet.


---

## Validated Production Models (`models/`)
...
*(Alternatively, click any of the one-click demo login buttons directly in the Patient Portal interface).*

---

## Cybersecurity Controls

* **Cryptographic Hashing:** Staff passwords and patient PINs are hashed using **PBKDF2-HMAC-SHA256** (100,000 iterations + 16-byte random salts).
* **IDOR Prevention:** Access to reports and prediction history is cryptographically bound to verified token claims. Cross-patient requests are rejected with `403 Forbidden`.
* **Parameterized SQL:** All queries use SQLite parameterization with `?` placeholders.
* **Malaria Upload Security:** Enforces 5MB maximum file size, MIME-type verification, OpenCV memory buffer decoding, and dimension boundary checks ($20\times20$ to $4096\times4096$).
* **Automated Online Backups:** Programmatic, non-blocking online database backups via the SQLite Backup API.
