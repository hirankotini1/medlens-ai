...
class TestHospitalOperationsSuite(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        # Use isolated test database to protect development/production data
    ...
    def test_15_patient_flow_calculation(self):
        ...
        self.assertEqual(flow_metrics.total_admissions, 303)  # 309 rows - 6 secondary duplicates = 303 unique verified admissions
    ...
    def test_22_existing_feature_regression(self):
        ...
        # 1. Health check & 5 ML models
        res_health = self.client.get("/health")
        self.assertEqual(res_health.status_code, 200)
        health_data = res_health.json()
        self.assertEqual(health_data["status"], "healthy")
        for disease in ["anemia", "dengue", "liver", "thyroid", "malaria"]:
            self.assertTrue(health_data["models_available"][disease], f"Model {disease} must be available")

        # 2. Public patients
        res_patients = self.client.get("/api/patients/public")
        self.assertEqual(res_patients.status_code, 200)

        # 3. Anemia ML prediction
        payload = {
