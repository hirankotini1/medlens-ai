...
def parse_csv_report(file_bytes: bytes) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    # Check if first row is a horizontal multi-column table: e.g. "Patient ID, Age, Gender, Hemoglobin, MCV, PLT"
    ...
    # Process vertical rows with potential 2-column metadata pairs and biomarker tables
    for row in rows:
        if len(row) == 2:
...
def parse_pdf_report(file_bytes: bytes) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    try:
        ...
        for page in reader.pages:
            # 1. Primary: Extraction with layout mode to preserve horizontal table alignment
            try:
            ...
            # 2. Secondary: Plain stream extraction
            try:
            ...
            # 3. Check for embedded raster images (for scanned PDFs)
    ...
    # Prioritize layout text if available and non-trivial
