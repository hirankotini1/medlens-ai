...
def register_appointment (req :AppointmentRegisterRequest ):
    ...
    try :
        data =req .model_dump ()
        data ['full_name'] = data.get ('full_name') or data.get ('name') or "Patient"
        data ['phone'] = data.get ('phone') or data.get ('contact') or ""
        data ['time_slot'] = data.get ('time_slot') or data.get ('appointment_time') or "10:00 AM - 10:30 AM"
        data ['reason_for_visit'] = data.get ('reason_for_visit') or data.get ('symptoms') or "General Health Consultation"
        if data.get('insurance_covered'):
