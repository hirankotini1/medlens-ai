...
# Metadata and Header field names that MUST NEVER be treated as laboratory biomarkers
...
# Missing-value placeholder strings that must NOT become valid laboratory measurements
PLACEHOLDER_STRINGS = {
    "-1", "-999", "-99", "999", "null", "none", "n/a", "na", "nil", "not available",
    "not reported", "unknown", "blank", "", "-", "--", "undefined", "neg", "pending"
}

# Canonical Clinical Reference Ranges & Standard Display Names
CANONICAL_REF_RANGES: Dict[str, Dict[str, Any]] = {
    ...
    # Copper & Metal Metabolism Biomarkers (Wilson Disease & Trace Elements)
    ...
    # Hemolysis, Enzyme & Iron Kinetics Biomarkers
...
PARAMETER_ALIASES: Dict[str, str] = {
    ...
    # Specialized & Alpha-1 Antitrypsin
    ...
    # Specialized Enzymology, Gammopathy & Porphyrins
...
# Documented Clinical Laboratory Critical Panic Limits
# Only values crossing these physiological limits trigger CRITICAL flags
...
def parse_source_reference_range(ref_str: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    # Exact single value target like "100%" or "100"
...
def calculate_biomarker_status(
    ...
    """
    # 1. Check explicit panic threshold first if configured
    ...
    # 2. Check source reference range or canonical range
    ...
    if min_v is not None and max_v is not None:
        norm_val = val
        # Unit scaling check for platelets in thousands vs absolute
    ...
    # If no reference intervals could be calculated, check explicit source status
    if src_status:
...
def normalize_param_name(raw_name: str) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
    ...
    # Word-boundary alias match (prevents false positives like 'alp' inside 'alpha' or 't3' inside compound words)
...
def extract_metadata_from_lines(lines: List[str]) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        for part in parts:
            ...
            # Combined Age and Gender expressions (e.g. "Age / Gender: 58 Yrs / Male", "58 Yrs / Male", "58/M")
            ...
            # Reversed combined expressions (e.g. "Male / 58 Yrs", "Gender/Age: Male / 58")
...
def extract_parameters_from_raw_items(raw_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in raw_items:
        ...
        # Prevent negative values for standard biological metrics
        if num_val < 0:
            continue
...
def extract_parameters_from_text(raw_text: str) -> List[Dict[str, Any]]:
    ...
    for line in lines:
        ...
        # Check for multi-line Reference Range or Status definitions following previous parameter
        ...
        # Filter out obvious metadata and table header titles
        ...
        # -------------------------------------------------------------
        # Strategy A: Multi-Column Delimited Table Parsing (Pipes, Tabs, Multi-spaces)
        # -------------------------------------------------------------
        col_candidates = None
        if "|" in line_clean:
        ...
        # -------------------------------------------------------------
        # Strategy B: Suffix-Isolated Inline Free-Text Regex Matching
        # -------------------------------------------------------------
        matched_alias = None
        matched_c_key = None
        alias_span = None
        ...
        # Strict suffix isolation: only parse characters AFTER the complete biomarker name
        suffix = line_clean[alias_span[1]:].lstrip(":= \t-")
        if not suffix:
            continue

        # Extract reference range from suffix (supporting commas, Unicode dashes, %, <, >, parenthesized ranges)
        ...
        # Remove commas before finding numeric value (e.g. 205,000 -> 205000)
        ...
        # Recognize units with spaces, including U/g Hb, mg/24h, cells/uL, /uL, %, etc.
...
def calculate_report_data_quality(
    ...
    # Extraction confidence reflects extraction fidelity and reference range completeness
    ...
    # Overall Quality: Focused disease panels (e.g. Thyroid with 3-5 markers, LFT with 6-8 markers)
    # with high confidence and low unmapped count are rated GOOD, not unfairly downgraded to FAIR.
