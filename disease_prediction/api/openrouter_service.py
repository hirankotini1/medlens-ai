...
def enforce_clinical_guardrails(
    ...
    for c in conditions:
        if isinstance(c, dict):
            ...
            # Filter out any treatment directives from follow-up steps
    ...
    # If conditions list is missing or parameters provided, populate from rare disease engine
    ...
    # Precautions guardrail (no drug prescriptions, treatment advice, or dosages)
...
Generate a structured clinical decision-support analysis adhering strictly to the JSON schema."""
    ...
    # Priority: Free AI models on OpenRouter (Gemma 4 31B Free, MiniMax Free, Nemotron Free, OpenRouter Free)
    ...
    for cand_model in candidate_models:
        payload = {
            ...
            "messages": messages,
            "temperature": 0.1,
            "max_tokens": 1400  # Limit response length for faster generation
        }
        try:
            # Fast failover connect_timeout=3s, read_timeout=8s for responsive multi-model cascade
...
def get_fallback_analysis(
    ...
    # -------------------------------------------------------------
    # Multi-Disease Rare & Unusual Pattern Screening
    # Evaluates multi-marker concordance across all defined profiles
    # -------------------------------------------------------------
    rare_screening = rare_disease_engine.evaluate_rare_disease_patterns(parameters, patient_meta)
    
    # If rare disease patterns were flagged, incorporate candidate condition cards into conditions list
    ...
    # -------------------------------------------------------------
    # 1. Microcytic / Iron-Deficiency Anemia Pattern
    ...
    # 2. Thrombocytopenia / Viral Pattern
    ...
    # 3. Hepatic Injury Pattern (if not already part of a recognized rare pattern)
