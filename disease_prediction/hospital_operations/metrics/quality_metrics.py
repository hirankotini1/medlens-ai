...
class DataQualityMetricsCalculator :
    ...
    conflicts :List [ConflictRecord ]
        ...
        total_records =len (his_records )+len (lab_records )+len (bed_records )


        duplicates_count =sum (1 for r in his_records if r .is_duplicate )
        missing_avail_count =sum (1 for r in bed_records if r .was_available_imputed )
        missing_days_count =5 
        unmatched_outpatients =len (set (r .canonical_patient_id for r in lab_records if r .is_outpatient ))
