...
SUPABASE_HOST = os.getenv("SUPABASE_DB_HOST", "aws-0-ap-southeast-2.pooler.supabase.com")
SUPABASE_PORT = int(os.getenv("SUPABASE_DB_PORT", "6543"))
SUPABASE_USER = os.getenv("SUPABASE_DB_USER", "postgres.qvupcpgoxdcfdqowwhls")
...
class SupabaseHospitalClient:
    ...
    # ---------------------------------------------------------
    # 1. STAFF AUTHENTICATION & SESSION
    # ---------------------------------------------------------
    @classmethod
    def authenticate_staff(cls, username: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            FROM staff_users
            WHERE (username = %s OR staff_id = %s) AND password_hash = %s AND status = 'Active';
            """, (username.strip(), username.strip(), pw_hash))
            staff = cur.fetchone()
            conn.close()
    ...
    # ---------------------------------------------------------
    # 2. BED INVENTORY & 3-TIER CAPACITY SUMMARY
    # ---------------------------------------------------------
    @classmethod
    ...
    # ---------------------------------------------------------
    # 3. PATIENT REGISTRATION & ADMISSIONS ERP
    # ---------------------------------------------------------
    @classmethod
    ...
    def create_admission(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Find an available bed in the requested tier if not explicitly specified
            ...
            # Check for existing active admission for this patient
            ...
            # Delete any pre-registration stub for this patient ID so we have a single canonical active admission row
    ...
    # ---------------------------------------------------------
    # 4. LABORATORY ORDERS & RESULT UPDATES
    # ---------------------------------------------------------
    @classmethod
    ...
    # ---------------------------------------------------------
    # 5. SCHEMA HARDENING & PERSISTENT BILLING
    # ---------------------------------------------------------
    @classmethod
